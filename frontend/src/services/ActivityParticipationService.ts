...
import { IActivityParticipation , ICreateActivityParticipation  } from "../types/global.typing";
...
const ActivityParticipationService = {
  getAllActivityParticipations: async (): Promise<IActivityParticipation[]> => {
    try {
      const response = await httpModule.get<IActivityParticipation[]>(API_ENDPOINT);
  ...
  getActivityParticipationById: async (activityParticipationId: number): Promise<IActivityParticipation> => {
    try {
  ...
  updateActivityParticipation : async (
    ...
    try {
      await httpModule.put(`${API_ENDPOINT}/${activityParticipationId}`, activityParticipationData);
    } catch (error) {
      throw new Error("Failed to update activity participation");
    }
  },

  deleteActivityParticipation : async (activityParticipationId: number): Promise<void> => {
    try {
